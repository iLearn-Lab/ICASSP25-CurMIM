...
cos_attention = 1
def get_cos_attention(x):
    cos_attention = x
def return_cos_attention():
    return cos_attention
# attention_gradients = []
# def get_array(attention_gradients):
#     attention_gradients = attention_gradients
    
# def get_attention_gradients(grad):
#     if len(attention_gradients)==0:
#         attention_gradients.append(grad)
#     else:
#         attention_gradients[0] = (grad + attention_gradients[0])/2

# def return_attention_gradients():
#     return attention_gradients[0]
...
class New_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale
        # attn.register_hook(get_attention_gradients) 
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
